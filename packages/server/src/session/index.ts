    ...
    return token.createAndSignToken(
      {
        ...
        updatedAt: now,
        activatedAt: now,
        expiresAt: now + token.getTokenTimeout(securityLevel, expiresIn),
    ...
    const newToken = await this.app.token.createAndSignToken(
      {
        ...
        updatedAt: now,
        activatedAt: now,
        expiresAt: now + this.app.token.getTokenTimeout(securityLevel, expiresIn),
        jwtTimeout: this.app.token.getJwtTimeout(securityLevel, tokenTimeout),
    ...
    const newToken = await this.app.token.createAndSignToken(
      {
        ...
        expiresAt: token.activatedAt + this.app.token.getTokenTimeout(targetLevel),
        jwtTimeout: this.app.token.getJwtTimeout(targetLevel),
    ...
    const tokenDoc = await this.app.token.createToken(
      {
        ...
        expiresAt: parent.activatedAt + this.app.token.getTokenTimeout(securityLevel),
        jwtTimeout: this.app.token.getJwtTimeout(securityLevel),
        refreshTimeout: this.app.token.getRefreshTimeout(securityLevel),
        confidential,
        remote,
